...
-- Funcion que dado un estado y un conjunto de formulas, indica si el conjunto se satisface.
satisfenConj :: Estado -> [Prop] -> Bool
satisfenConj i []     = True
satisfenConj i (x:xs) = and ((satisfen i x):[satisfenConj i xs])

-- Funcion que devuelve una lista con todos los estados de cada formula del conjunto.
estadosConj :: [Prop] -> [Estado]
estadosConj []     =  []
estadosConj (x:xs) =  estados x ++ estadosConj xs

-- Funcion que indica si una formula es consecuencia logica de un conjunto de formulas.
...
fnc1 = fnc $ Or (And (Neg (P 'p')) (P 'q')) (And (P 'r') (P 's'))
-- Devuelve: (((¬ 'p' v 'r') ∧ (¬ 'p' v 's')) ∧ (('q' v 'r') ∧ ('q' v 's'))) (Es correcto :D)
